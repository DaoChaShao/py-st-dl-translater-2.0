...
class SeqToSeqTaskLSTM(BaseSeqNet):
    ...
    def _merge_bidirectional_hidden(self, hidden: Tensor | tuple[Tensor, Tensor]) -> Tensor | tuple[Tensor, Tensor]:
        ...
        batch_size = h.size(1)
        h = h.view(self._C, self._num_directions, batch_size, self._M)
        c = c.view(self._C, self._num_directions, batch_size, self._M)

        if self._method == "average":
            h_merged = h.mean(dim=1)  # [num_layers, batch_size, hidden_size]
            c_merged = c.mean(dim=1)
            return (h_merged, c_merged)
        elif self._method == "concat":
            h_merged = cat([h[:, 0], h[:, 1]], dim=-1)  # [num_layers, batch_size, hidden_size*2]
            c_merged = cat([c[:, 0], c[:, 1]], dim=-1)
            return (h_merged, c_merged)
        else:
            raise ValueError(f"Unknown merge method: {self._method}")

    @final
    def _forward_with_attention(self, decoder_input: Tensor, decoder_hidden: tuple[Tensor, Tensor],
                                encoder_outputs: Tensor) -> Tensor:
        """ Forward pass with attention mechanism (LSTM version) """
        ...
        for t in range(tgt_len):
            ...
            # Get the first item from the hidden for attention
            hidden_for_attention = decoder_hidden[0] if isinstance(decoder_hidden, tuple) else decoder_hidden

            # Attention step
            combined = None
            match self._attention_type:
                case "single":
                    attn_weights, context = self._attention(hidden_for_attention, encoder_outputs)
                    # Combine decoder output and context
                    combined = cat([decoder_output, context.unsqueeze(1)], dim=-1)
                case "multi":
                    # Transpose for attention computation
                    query = hidden_for_attention.transpose(0, 1)  # [batch_size, 1, hidden_size]
